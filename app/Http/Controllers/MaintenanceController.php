...
//******get the previous data
    public function getPrevious($val){
        ...
        // checking the date
            if(!empty($equipment)){
                $equipDetails = decrypt($equipment->details);
                
                $details = [];
                foreach ($equipDetails as $key => $value) {

                   $value["operational"] = isset($value["actual"])?$value["actual"]:'';
                   $value["repair"] = '';
                   $value["dispose"] = '';
                   $details[] = $value;
                }
                $master = $this->masterTable($details,'insert');
                ...
                $insert->master = encrypt($master);
                $insert->result = encrypt($result);
            ...
            return $this->getMaintenance($val);
        } catch (\Exception $e) {
            return response()->json(['error'=>'Failed' . $e->getMessage()],500);
        }
    }
    ...
    public function updateMaintenance(Request $request){
        try {
            ...
            $master = $this->masterTable($request,'update');
            $result = $this->resultData($master);
            ...
            $update = Maintenance::where('id',$request->id)->update($val);
    ...
    public function masterTable($request,$stats){
        if($stats != 'insert'){
            ...
            if ($matchedObject) {
                ...
                foreach ($details as $current) {
                    if (($current['sub'] === $val || $current['category'] === $val) && empty($current['Title'])) {
                        $required += (int) ($current['actual'] ?? 0);
                        $actual += (int) ($current['operational'] ?? 0);
                    }
                }
                ...
                foreach ($details as $current) {
                    if (empty($current['na'])) {
                        if($current['Info'] === 'primary'){
                            $required1 += (int) ($current['actual'] ?? 0);
                            $actual1 += (int) ($current['operational'] ?? 0);
                        }else{
                            $required2 += (int) ($current['actual'] ?? 0);
                            $actual2 += (int) ($current['operational'] ?? 0);
                        }
                    }
                }
                    $result[] = array(
