    ...
    export default {
        ...
        methods: {
            ...
            async exportTable() {
             ...
             val.tableData.forEach(data => {
                     ...
                     let excelRow = worksheet.addRow(row);
                        ...
                        const operationalIndex = selectedColumns.indexOf('operational');
                        const repairIndex = selectedColumns.indexOf('repair');
                        const disposeIndex = selectedColumns.indexOf('dispose');
                        ...
                        if (data.operational && operationalIndex !== -1) {
                            // Set the horizontal alignment to center for the "operational" column
                            excelRow.getCell(operationalIndex + 1).alignment = { horizontal: 'center' };
                        }
                        if (data.repair && repairIndex !== -1) {
                            // Set the horizontal alignment to center for the "repair" column
                            excelRow.getCell(repairIndex + 1).alignment = { horizontal: 'center' };
                        }
                        if (data.dispose && disposeIndex !== -1) {
                            // Set the horizontal alignment to center for the "dispose" column
                            excelRow.getCell(disposeIndex + 1).alignment = { horizontal: 'center' };
                        }
             });
            ...
            worksheet.eachRow((row) => {
                ...
                link.href = url;
                link.download = `${this.excelData.team}-${this.excelData.monthString}-Maintenance List.xlsx`;
